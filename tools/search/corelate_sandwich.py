...
class EvolutionSearcher(object):
    ...
    def __init__(self, args):
        ...
        self.log_dir = './sandiwch_supernet'
    ...
    def is_legal(self, arch, **kwargs):
        ...
        info['size'] = size
        info['cfg'] = cfg
        self.flops_l.append(size)
        self.params_l.append(fp)
        del size, fp, cfg
        ...
        self.map_sandwich.append(map)
        print(self.arch_l, self.map_sandwich, self.flops_l, self.params_l)
        torch.cuda.empty_cache()
        if map:
            info['map'] = map[0]
            info['map_list'] = map
    ...
    def get_random(self, num): # num=population_num
        ...
        for i in range(200):
            ...
            if not self.is_legal(cand):
                continue
            self.arch_l.append(cand) # cand_tuple集合[]
            print(self.arch_l,self.map_sandwich,self.flops_l,self.params_l)
            logging.info(
                'arch:{}, AP {}, {} M, {} GFLOPS'.format(
                    self.arch_l,
                    self.map_sandwich,
                    self.params_l,
                    self.flops_l,
